...
indir='/data/home/mass/fscucchia/Bermuda/output/Diamond/Diamond_prot5'
files_pattern='(.*)(?=\\.m8$)'
design_file='/data/home/mass/fscucchia/Bermuda/scripts/design/metadata_diamond_Past.csv'

design_factor='group'
design_factor_levels=c("adult_meso","planu_meso", "adult_shal", "planu_shal")
...
save.image(file=paste0(outidx,'.1-prot5.RData')) #until here
...
write.table(speciesFreq2,sep="\t",file=paste0(outidx,".species-freq-prot5.txt"),row.names=F,quote=F)
...
for(k in 1:length(clades_name)){
    ...
    write.table(speciesFreq_byClade_per[[k]],sep="\t",file=paste0(outidx,".species-freq-prot5.",clades_name[k],".txt"),row.names=F,quote=F)
    
    mlt1=melt(speciesFreq_byClade_per[[k]],id.vars='species')
    mlt2=merge(mlt1,t_design1,by.x='variable',by.y='sample',all.x=T,all.y=T)
    pdf(paste0(outidx,".species-freq-prot5.",clades_name[k],".pdf"),width=10)
    hm1 = ggplot(data = mlt2, mapping = aes(x = variable, y = species, fill = value)) +
        geom_raster() + ggtitle('proportion of mapped reads') +
        theme(axis.text.x=element_text(angle = 45)) + facet_grid(cols = vars(group),scale='free') +
    ...
    pdf(paste0(outidx,".species count-prot5.",clades_name[k],".pdf"),width=10)
